...
const AdminAppointments = () => {
    ...
    const getStatusColor = (status) => {
        switch (status) {
            ...
            case 'CONFIRMED': return 'bg-emerald-900/50 text-emerald-400 border-emerald-800';
    ...
    return (
        <div>
            <h1 className="text-2xl font-bold text-white mb-6">System Appointments</h1>
                            ...
                            <tbody className="divide-y divide-slate-700">
                                {appointments.map((apt) => (
                                    <tr key={apt.id} className="hover:bg-slate-700/50 transition-colors">
                                        <td className="p-4 text-white font-medium">{apt.patient?.name}</td>
                                        <td className="p-4 text-slate-300">Dr. {apt.doctor?.name}</td>
                                        <td className="p-4 text-slate-400 text-sm">{apt.department?.name}</td>
                                        <td className="p-4 text-sm text-slate-300">
                                            <div className="flex items-center whitespace-nowrap">
                                                <Calendar className="w-4 h-4 mr-1 text-slate-500" />
                                                {dayjs(apt.appointmentDate).format('MMM D, YYYY')}
                                            </div>
                                            <div className="flex items-center text-slate-400 mt-1 whitespace-nowrap">
                                                <Clock className="w-4 h-4 mr-1 text-slate-500" />
                                                {apt.startTime.substring(0, 5)} - {apt.endTime.substring(0, 5)}
                                            </div>
                                        </td>
                                        <td className="p-4">
                                            <span className={`px-2 py-1 rounded text-xs font-semibold border ${getStatusColor(apt.status)}`}>
                                                {apt.status}
                                            </span>
                                        </td>
                                        <td className="p-4 text-right">
                                            {(apt.status === 'BOOKED' || apt.status === 'CONFIRMED') && (
                                                <button
                                                    ...
                                                    className="inline-flex items-center px-2 py-1 bg-red-900/40 hover:bg-red-900/80 text-red-400 border border-red-800 rounded text-xs font-medium transition-colors"
                                                    title="Force Cancel"
                                                >
